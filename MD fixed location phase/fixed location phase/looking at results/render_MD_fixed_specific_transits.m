...
for omega_index = 1:numel(omega_end_range)
    ...
    if record_video
        if view_type == 2
            v = VideoWriter("fixed_MD_"+string(round(omega_end_range(omega_index)))+"_topview", 'MPEG-4');
        else
            v = VideoWriter("fixed_MD_"+string(round(omega_end_range(omega_index))), 'MPEG-4');
        end
        v.FrameRate = 60;
        open(v);
    end
    ...
    for ind_frame = 1 : numel(animation_time_series)
        ...
        for n=1:numel(sim_labels)
            ...
            if t > min(statematrix_spec(:,1)) && t < max(statematrix_spec(:,1)) && ismember(pso_results_total.(sim_labels(n)).TA_omega(2), omega_end_range(omega_index))
                ...
                rocket_desc = "MDR-"+ n + newline + "$\nu_l$ = " + round(TA_omega(1),2) + "$^{\circ}$" + newline;
        ...
        if view_type == 1
            ...
            legend( "fixed MD phase transits $ \forall \enspace \Omega_t$ = "+ omega_end_range(omega_index)+"$^{\circ}$" + newline +"time: "+round(t/(3600*24),3) +" days", ...
