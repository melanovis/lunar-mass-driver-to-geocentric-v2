...
omega_indexes = [1:numel(omega_end_range)];
view_type = 2;
...
for n=1:numel(sim_labels)
    ...
    if ismember(pso_results_total.(sim_labels(n)).TA_omega(2), omega_end_range(omega_indexes))
        for m = 1:4
...
if record_video
    v = VideoWriter("fixed_MD_recording_all_together_topview", 'MPEG-4');
    v.FrameRate = 60;
    open(v);
end
...
for ind_frame = 1 : numel(animation_time_series)
    ...
    for n=1:numel(sim_labels)
        ...
        plot_rocket = false;
        if t > min(statematrix_spec(:,1)) && t < max(statematrix_spec(:,1)) && ismember(pso_results_total.(sim_labels(n)).TA_omega(2), omega_end_range(omega_indexes))
            ...
            % rocket_desc = "sim rocket "+ n + newline + "$\nu$ = " + TA_omega(1) + ", " + "$\Omega$ = " + TA_omega(2) + newline;
            % rocket_desc = rocket_desc + "$\Delta$v = "+ round(dv_total/1e3,3) + " km/s" + newline;
            % rocket_desc = rocket_desc + "T/W $\approx$ " + round(TW_ratio,3) + newline;
            rocket_desc = "MDR-"+n;

            %plot_obj(2) = text(state_interp(2),state_interp(3),state_interp(4), [" ", rocket_desc], Interpreter="latex", FontSize=9, VerticalAlignment="cap", Color = [1,1,1]);
        end
    end    
    ...
    plot_obj(3) = plot3(earth_init_statematrix(:,2),earth_init_statematrix(:,3),earth_init_statematrix(:,4),"b");
    plot_obj(4) = plot3(lunar_init_statematrix(:,2),lunar_init_statematrix(:,3),lunar_init_statematrix(:,4),"w");
    ...
    luna_obj = surf(surface_map_x,surface_map_y,surface_map_z,edgealpha=0.1,FaceColor=[repelem(0.5,3)]);

    plot_obj(5) = text(luna_state(1),luna_state(2),luna_state(3), [""," Luna"], Interpreter="latex", FontSize=12,VerticalAlignment="cap",color=[1,1,1]);
    plot_obj(6) = scatter3(luna_state(1),luna_state(2),luna_state(3),200,"w+");
    ...
    for n=1:numel(omega_end_range)
        if ismember(n,omega_indexes)
            ...
            target_orbits(n) = plot3(target_orbit_statematrix(:,2),target_orbit_statematrix(:,3),target_orbit_statematrix(:,4),"w");
        end
    end
    ...
    if view_type == 1
        ...
        legend( "fixed MD phase transits" + newline + "time: "+round(t/(3600*24),3) +" days", ...
