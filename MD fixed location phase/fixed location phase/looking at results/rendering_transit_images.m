...
for n=1:numel(sim_labels)
    ...
    dt_total = pso_results_total.(sim_labels(n)).results_struct.dt_total;
    max_acc =  max(pso_results_total.(sim_labels(n)).results_struct.acc_series);

    legend( "fixed MD phase transits - MDR " + erase(sim_labels(n),"ind_s_") + newline + ...
        "$\nu_l$ = "+ round(TA_omega(1),2)+"$^{\circ}$, "+"$\Omega_t$ = "+ TA_omega(2)+"$^{\circ}$" + newline + ...
        "transit time: " + round(dt_total/(3600*24),3) + " days" + newline + ...
        "launch v = " + round(launch_params(1)/1e3,3) + " km/s" + ", rocket $\Delta$v = " + round(dv_total/1e3,3)+" km/s" + ", "+"max T/W = " + round(max_acc/9.81,3) + newline + ...
        "launch $\phi$ = " + round(launch_params(3),2)+"$^{\circ}$" + ", launch $\lambda$ = " + round(launch_params(2),2)+"$^{\circ}$" + ...
