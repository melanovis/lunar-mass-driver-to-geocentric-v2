function [global_best_struct, PSO_killed,steps_forward, view_struct] = PSO_handler(target_altitude, TA_start_norm, target_omega, max_iters, dv_kill_markers, plot_result, recorder_struct, view_struct)
    ...
    population = 8*round(interp1([0,1],[15,25],rand()));
    phi_1 = 2.05;
    phi_2 = 2.05;
    phi = phi_1+phi_2;
    kappa = 1;
    ...
    checkgood_threshold = round(interp1([0,1],[10,600],rand()));
    %checkgood_threshold = 5;
    ...
    while init_checkgood_quantity < checkgood_threshold
        ...
        parfor n=1:population
            ...
            [return_struct] = transfer_handler(input_vector, TA_start_norm, target_radius, target_omega, earth_init_statematrix, lunar_init_statematrix, earthluna_orbital_period);
    ...
    while iter <= max_iters
        ...
        if update_plot && plot_result
            ...
            plot3(transit_statematrix(:,2),transit_statematrix(:,3),transit_statematrix(:,4),"w",linewidth=1.5)
            scatter3(transit_statematrix(ind_transitions,2),transit_statematrix(ind_transitions,3),transit_statematrix(ind_transitions,4),20,"r","filled")
            burn_list = global_best_struct.burn_dv_series;
            unit_vec_scale = 0.3e8;
            for n=1:height(burn_list)
                text(transit_statematrix(ind_transitions(n),2),transit_statematrix(ind_transitions(n),3),transit_statematrix(ind_transitions(n),4),"  "+string( round(norm(burn_list(n,:))/1e3,3) ) + " km/s" , color=[1,0,0])
                burn_vector = burn_list(n,:);
                burn_vector = burn_vector./norm(burn_vector);
                quiver3( transit_statematrix(ind_transitions(n),2),transit_statematrix(ind_transitions(n),3),transit_statematrix(ind_transitions(n),4), burn_vector(1)*unit_vec_scale,burn_vector(2)*unit_vec_scale,burn_vector(3)*unit_vec_scale, color=[1,0,0])
                plot3([repelem(transit_statematrix(ind_transitions(n),2),2)],[repelem(transit_statematrix(ind_transitions(n),3),2)],[0,transit_statematrix(ind_transitions(n),4)],"--r",linewidth=0.1)
            end
            ...
            legend("$\Delta$v: "+string(round(global_best_dv./1e3,3))+" km/s", "$\Delta$t: " + string(round(global_best_dt / (3600*24), 3 ))+" days", 'textcolor', "w", Interpreter="latex", FontSize=20, location = "northwest")
            legend boxoff
    
            set(findall(gcf,'-property','FontSize'), 'FontName', 'Times')
    
            if recorder_struct.record
                xlim([-1,1].*4.5e8)
                ylim([-1,1].*4.5e8)
                zlim([-1,1].*2e8)
            end
            ...
            view_struct.view_az = view_struct.view_az + 5/60;

            view([view_struct.view_az, view_struct.view_el])

            if recorder_struct.record
                frame = getframe(gcf);
                for n=1:3
                    frame_new(:,:,n) = uint8(imresize(squeeze(frame.cdata(:,:,n)), [nan,recorder_struct.frame_width]));
                end
                writeVideo(recorder_struct.video_obj,frame_new)
            end
        ...
        % PSO kill conditions
        if iter > 12 && isinf(global_best_fitness)
            PSO_killed = true;
            break
        end
