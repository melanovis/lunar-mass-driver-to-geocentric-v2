function [return_struct] = transfer_handler(input_vector, TA_start_norm, target_radius, target_omega, earth_init_statematrix, lunar_init_statematrix, earthluna_orbital_period)
...
eject_v = interp1([0,1],[2.3e3, 2.6e3], input_vector(2) ); %m/s
...
%launch params, hard specified
launch_long = -34.775;
launch_lat = 9.378;
launch_azimuth = 45;
launch_elevation = 0.0148;

earth_SOI_approx = 1.5e9;

manuever_matrix = input_vector(6:6 + max_manuevers*6 -1 );
...
for n=1:manuevers
    ...
    target_r = interp1([0,1],[earth_radius+800e3, earth_SOI_approx/1.5],manuever_matrix(n,2));
...
ER3BP_timespan = [0,earthluna_orbital_period * 3];
...
function [state_out, inds_between] = interp_statematrix_via_time(statematrix,t_target)
    ...
    inds_between = sort(inds_between);
    if statematrix(inds_between(1),row_time) == statematrix(inds_between(2),row_time)
        if inds_between(1) == 1
            inds_between(2) = inds_between(2)+1;
        else
            inds_between(1) = inds_between(1)-1;
        end
    end
    for n=1:width(statematrix)
