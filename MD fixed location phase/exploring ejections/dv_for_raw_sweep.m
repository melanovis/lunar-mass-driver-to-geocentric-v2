...
omega_range = linspace(0,360,200);

for n=1:numel(omega_range)

    target_omega = omega_range(n);
    ...
    target_orbit_stateinital = [r_target_offset, v_target_offset] ;
    ...
    lunar_normal = cross(lunar_init_statematrix(1,2:4), lunar_init_statematrix(round(end/3),2:4));
    lunar_normal = lunar_normal/norm(lunar_normal);
    
    target_normal = cross(target_orbit_statematrix(1,2:4), target_orbit_statematrix(round(end/3),2:4));
    target_normal = target_normal/norm(target_normal);
    
    if target_normal(3) < 0
        target_normal = -target_normal;
    end
    if lunar_normal(3) < 0
        lunar_normal = -lunar_normal;
    end
    
    theta = acosd( dot(lunar_normal,target_normal) / (norm(lunar_normal)*norm(target_normal)) );
    
    target_v = norm(target_v_initial);
    
    v_inc_change =  [
    cosd(theta), -sind(theta)
    sind(theta), cosd(theta)
    ] * [target_v; 0];
    v_inc_change = v_inc_change.';
    
    manuever_v = 2.5e3;
    
    dv_total(n) = norm( [target_v-manuever_v,0] - v_inc_change );
...
grid on
axis tight
plot(omega_range,dv_total./1e3,"w",LineWidth=1.5)
ylim("padded")
ax = gca;
ax.FontSize = 20;

xlabel("lunar-target $\Delta \Omega$ (degrees)", Interpreter="latex", FontSize=20)
ylabel("rendezvous $\Delta$v (km/s)", Interpreter="latex", FontSize=20)
title("Effect of LEO nodal precession on single-burn rendezvous $\Delta$v assuming an idealised 2.5 km/s retrograde burn", Interpreter="latex", FontSize=20)
ax = gca;
...
im_raw = getframe(gcf);
imwrite(im_raw.cdata, "effect_of_precession.png");
