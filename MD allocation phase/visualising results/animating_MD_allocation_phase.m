...
if record_video
    v = VideoWriter("MD_allocation_recording", 'MPEG-4');
    v.FrameRate = 60;
    open(v);
end
...
for ind_frame = 1 : numel(animation_time_series)
    ...
    for n=1:numel(sim_labels)
        ...
        if t > min(statematrix_spec(:,1)) && t < max(statematrix_spec(:,1)) && ismember(pso_results_total.(sim_labels(n)).TA_omega(2), omega_end_range(omega_indexes))
            ...
            rocket_desc = "MDR-"+n + newline + round(dv_total/1e3,3) + " km/s" + newline + "T/W $\approx$ " + round(TW_ratio,3);

            text(state_interp(2),state_interp(3),state_interp(4), [" ", rocket_desc], Interpreter="latex", FontSize=10, VerticalAlignment="cap", Color = [1,1,1])
        end
    end    
    ...
    text(luna_state(1),luna_state(2),luna_state(3), [""," Luna"], Interpreter="latex", FontSize=12,VerticalAlignment="cap",color=[1,1,1])
    ...
    if view_type == 1
        ...
        legend( "MD allocation phase transits for $\Omega_t$ = " + omega_end_range(omega_indexes) + "$^{\circ}$" + newline + "time: "+round(t/(3600*24),3) +" days", ...
    ...
    if record_video
        frame = getframe(gcf);
        for n=1:3
            frame_new(:,:,n) = uint8(imresize(squeeze(frame.cdata(:,:,n)), [nan, 1920*1.5 ]));
        end
