...
if record_video
    v = VideoWriter("MD_allocation_recording_all_together", 'MPEG-4');
    v.FrameRate = 60;
    open(v);
end
...
for ind_frame = 1 : numel(animation_time_series)
    ...
    for n=1:numel(sim_labels)
        ...
        if t > min(statematrix_spec(:,1)) && t < max(statematrix_spec(:,1)) && ismember(pso_results_total.(sim_labels(n)).TA_omega(2), omega_end_range(omega_indexes))
            ...
            text(state_interp(2),state_interp(3),state_interp(4), [" ", rocket_desc], Interpreter="latex", FontSize=9, VerticalAlignment="cap", Color = [1,1,1])
        end
    end    
    ...
    scatter3(luna_state(1),luna_state(2),luna_state(3),200,"w+")
    ...
    if view_type == 1
        ...
        legend( "MD allocation phase transits" + newline + "time: "+round(t/(3600*24),3) +" days", ...
