function [return_struct] = transfer_handler(input_vector, TA_start_norm, target_radius, target_omega, earth_init_statematrix, lunar_init_statematrix, earthluna_orbital_period, min_manuever_cycle)
...
manuevers = round(interp1([0,1],[min_manuevers-0.49, max_manuevers+0.49], input_vector(1) ));

eject_v = interp1([0,1],[2.32e3,3.5e3], input_vector(2) ); %m/s

dt_final = input_vector(3);
...
launch_azimuth = interp1([0,1],[0,180], input_vector(8) );
launch_elevation = 10 ^ interp1([0,1],[-6,-0.3], input_vector(9) ); %epsilon, relative to surface tangent
...
if rem(target_omega,360)==0
    ...
    %finding new launch lat and long
    launch_r_norm = (launch_r./norm(launch_r));
    tmp_x = dot(launch_r_norm,null_norm);
    tmp_y = dot(launch_r_norm,equator_90_norm);
    tmp_z = dot(launch_r_norm,pole_norm);
    launch_long = atan2(tmp_y, tmp_x);
    launch_lat = atan2(tmp_z, sqrt(tmp_x^2 + tmp_y^2));
end
...
for n=1:manuevers
    ...
    target_h = interp1([0,1],[-1e8, 1e8],manuever_matrix(n,4));

    dt_burn_series(n,1) = interp1([0,1],[0, 1-1e-6],manuever_matrix(n,1));

    dt_lambert_series(n,1) = manuever_matrix(n,5);

    throttle_series(n,1) = 10 ^ interp1([0,0.9],[-8, 0.5],manuever_matrix(n,6));
    
    r_point = [target_r*cosd(target_theta), target_r*sind(target_theta), target_h];
    if rem(target_omega,360) ~= 0 
        target_series(n,:) = r_point;
    else
        %constrain target points to lunar OP
        r_point_proj = r_point - dot(r_point, lunar_OP_normal)*lunar_OP_normal;
        target_series(n,:) = r_point_proj;
    end
end
% target_series
% dt_lambert_series
...
ER3BP_timespan = [0,earthluna_orbital_period*2.2];
...
for n=1:30
    ...
    if ER3BP_timespan(1) == ER3BP_timespan(2)
        break
    end
...
while theta < impulsive_assumption_angle
    r_spec = total_transit_statematrix(ind_spec,2:4);
...
if shooting_good
    thrust_penalty = exp( -max(acc_req_series)/5e3 ); %fitness component for thrust 
    time_penality = exp( -log10(transit_time)/1e6 ); %fitness component for total transit time
...
eject_velocity_penalty = exp( -eject_v/1e7);

fitness = 6e5/(dv_total + (absent_lastleg_modifier) );

fitness = fitness * shooting_penalty * collision_penalty * eject_velocity_penalty * deep_space_penalty * thrust_penalty;
...
return_struct = setfield(return_struct,"manuevers",manuevers);
