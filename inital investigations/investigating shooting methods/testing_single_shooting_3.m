...
r_rocket_start = [interp1([0,1],[-9,9],rand())*10^interp1([0,1],[6,7.5],rand()), interp1([0,1],[-9,9],rand())*10^interp1([0,1],[6,7.5],rand()), 0];

r_target = [0,-earth_radius-1200e3,0];
...
[v_1,v_2,delta_t] = lambert_solve(r_rocket_start, r_target, 0.1, G*earth_mass);
v_guess_inital = v_1;
...
initalguess_time = scene_rocket_statematrix(ind_target_min,1);

ER3BP_timespan = t_start + [0, initalguess_time];
...
for n=1:30
    ...
    [jacobian, residual_matrix] = fetch_jacobian_and_residual(r_target, ER3BP_timespan, ER3BP_state_inital, lunar_mass, earth_mass, error_tolerance);
...
function [jacobian,residual_matrix] = fetch_jacobian_and_residual(r_target, ER3BP_timespan, ER3BP_state_inital, lunar_mass, earth_mass, error_tolerance)
