...
[v_1,v_2,delta_t] = lambert_solve(r_rocket_start, r_target, 0.5, G*earth_mass);
v_guess_inital = v_1;

rocket_start_statematrix = [t_start, r_rocket_start, v_guess_inital];
ER3BP_state_inital = [rocket_start_statematrix(2:7), earth_start_statematrix(2:7), lunar_start_statematrix(2:7)];

%this has failed, we need to have time be a free variable too
...
for n=1:7
    ...
    [rv_jacobian, residual_matrix] = fetch_jacobian_and_residual(r_target, ER3BP_timespan, ER3BP_state_inital, lunar_mass, earth_mass, error_tolerance);
    
    fprintf("-------\n")
    rv_jacobian
    norm(residual_matrix)

    [u_mat,s_mat,v_mat] = svd(rv_jacobian);
    v_step = -v_mat * (s_mat \ (u_mat' * residual_matrix))
    %v_step = -rv_jacobian\residual_matrix
    ...
    r_target_approach = sqrt(sum((scene_rocket_statematrix(:,2:4) - r_target).^2, 2));
    [~,ind_target_min] = min(r_target_approach );
    r_closest_approach = scene_rocket_statematrix(ind_target_min,2:4);
    plot3(scene_rocket_statematrix(1:ind_target_min,2),scene_rocket_statematrix(1:ind_target_min,3),scene_rocket_statematrix(1:ind_target_min,4))
...
scatter3(r_target(1),r_target(2),r_target(3),"magenta","x")
...
function [rv_jacobian,residual_matrix] = fetch_jacobian_and_residual(r_target, ER3BP_timespan, ER3BP_state_inital, lunar_mass, earth_mass, error_tolerance)

    %naive approach (no clue what i'm doing)

    peturb_factor = 1e-6;

    t_start = ER3BP_state_inital(1);
    r_start = ER3BP_state_inital(2:4);
    v_start = ER3BP_state_inital(5:7);

    rv_jacobian = zeros(3,3);
    ...
    for n=1:4
        if n==1
            ...
            ER3BP_state_test = ER3BP_state_inital;
            ER3BP_state_test(4+n-1) = ER3BP_state_test(4+n-1) + peturb_factor;
        end

        [scene_timerange, scene_statematrix] = ode113(@(scene_timerange, scene_statematrix) threebody_ODE(scene_timerange, scene_statematrix, earth_mass, lunar_mass), ER3BP_timespan, ER3BP_state_test ,odeset('Reltol',error_tolerance, 'OutputFcn', @stop_too_slow));
        scene_timerange = scene_timerange + t_start;
        scene_rocket_statematrix = [scene_timerange, scene_statematrix(:,1:6)];
        
        %this is likely what is causing problems
        r_target_approach = sqrt(sum((scene_rocket_statematrix(:,2:4) - r_target).^2, 2));
        [~,ind_target_min] = min(r_target_approach );
        r_closest_approach = scene_rocket_statematrix(ind_target_min,2:4);

        if n==1
            r_closest_original = r_closest_approach;
            residual_matrix = (r_closest_original - r_target).';
        else
            rv_jacobian(:,n-1) = ((r_closest_original - r_closest_approach)/peturb_factor).';
        end
