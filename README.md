...
- Location, the latitude (φ) and longitude (λ) on the moon where we place the exit of the MD.

The following animation takes a look at how changing even a single one of these parameters, the gun's azimuth, greatly changes the resultant trajectory across the lunar orbit.
...
In order to answer this question we decided to split this simulation project into two phases. The first phase, which we’ll call the ‘allocation phase’ would have far less samples and leave these parameters free. We’ll then look at where the optimisation system places the MDs for each ν-Ω sample to see if any patterns or rough regions emerge. As an additional note here, the parameter optimisation system chosen for this project is particle swarm optimisation (PSO) with a periodic agitation stage on the majority of low-preforming swarm particles when fitness plateaus are detected.
...
The resulting transits are shown all together in this animation, note the red vectors indicate the direction of burns (note though that compression and the increased speed of this video makes this hard to see, sorry!).
...
So how are these sites found? This project looks at nine remote-sensing based abundance maps from various papers and then took the gradient of lunar heightmap to indicate terrain flatness. A PSO system then system which tweaks the threshold of each map, eg saying ‘only show me sites in the top x percentile of abundance this mineral’. This system then combines all these maps and continues to tweak each threshold until it maximises the mean abundance of each material while also minimizing terrain flatness. The results are the following spots.
...
This shows that a lunar CAM is very much possible. Mass can be constantly exported to the emerging macro-infrastructure markets of LEO constantly like a railway network. These are very much nontrivial Δv and T/W figures, but they are not outside the reasonable bounds of application for something like a potentially radically inexpensive mass-produced Al+O2 thruster operating at an isp of something like 230 seconds. 
