...
while ~check
    ...
    state_luna_start = transit_statematrix(1,14:19);
...
[surface_map_x,surface_map_y,surface_map_z] = ellipsoid(state_luna_start(1),state_luna_start(2),state_luna_start(3),lunar_radius,lunar_radius,lunar_radius,64);
earth_obj = surf(surface_map_x,surface_map_y,surface_map_z,edgealpha=0.1,FaceColor=[0.5,0.5,0.5]);

set(gcf, 'Color', [1,1,1])
set(findall(gcf,'-property','FontSize'), 'FontName', 'Times')

% xlim(state_luna_start(1)+[-2e7,2e7])
% ylim(state_luna_start(2)+[-2e7,2e7])
% zlim(state_luna_start(3)+[-2e7,2e7])
% view([20,20])
view([0,90])
drawnow()
hold off
