...
record_video_test = true;

if record_video_test
    v = VideoWriter("record_test", 'MPEG-4');
    v.FrameRate = 60;
    open(v);
end
...
c1 = chi*phi_1;
c2 = chi*phi_2;
max_velocity = (var_max-var_min)*0.5;
...
while iter <= max_iters
    ...
    for n=1:population
        if particle(n).best.fitness > global_best.fitness
            ...
            if global_best.fitness > best_fit_prev
                input_vector = particle(n).position;
                [return_struct] = transfer_handler(input_vector, TA_start_norm, target_radius, target_omega, earth_init_statematrix, lunar_init_statematrix, earthluna_orbital_period);
                if all(return_struct.check_matrix)
                    %update global best
    ...
    fprintf("\n completed iter %i, %3.2f, %3.2f km/s, %i.\n", iter, global_best_fitness , global_best_dv / 1e3, steps_forward )
